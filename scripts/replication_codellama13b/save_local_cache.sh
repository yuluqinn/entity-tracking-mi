...
#$ -l gpu_memory=40G
#$ -o logs/$JOB_ID_replication_codellama13b_save_local.log
#$ -j y
#$ -m e
#$ -M yuluqinn24@gmail.com

# === CodeLlama-13B REPLICATION (paper baseline) — local-probe cache ===
# Caches last-token representations (all 41 layers) for the exclude_empty split,
# used by both the local (binary) and mention probes. Uses the original nnsight_env.
