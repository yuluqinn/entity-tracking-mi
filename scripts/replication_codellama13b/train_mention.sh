...
#$ -o logs/$JOB_ID_replication_codellama13b_train_mention.log
#$ -j y
#$ -m e
#$ -M yuluqinn24@gmail.com

# === CodeLlama-13B REPLICATION (paper baseline) — mention (control) probes, layers 1-40 ===
# Reuses the local-probe cache. NOTE: the correct flag is --mention; the legacy
# scripts' "--exp_name mention" is silently dropped and trains a local probe instead.
...
do
    python probe_experiments/train_probe.py \
        ...
        --exclude_empty \
        --condition_on the \
        --checkpoint_root probe_experiments/probe_checkpoints/codellama-13b-replication/mention_the \
