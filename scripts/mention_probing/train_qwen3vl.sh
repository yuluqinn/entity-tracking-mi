...
#$ -o logs/$JOB_ID_mention_qwen3vl_train.log
#$ -j y
#$ -m e
#$ -M yuluqinn24@gmail.com

# Mention probe = control probe: same cached last-token activations as the local
# probe, but labels are "was object o mentioned anywhere in the story so far".
# Reuses the local-probe representation cache — no save/caching stage needed.
# NOTE: the flag is --mention; the legacy codellama scripts' "--exp_name mention"
# is silently dropped by parse_known_args and trains a local probe instead.
...
do
    python probe_experiments/train_probe.py \
        ...
        --exclude_empty \
        --condition_on the \
        --checkpoint_root probe_experiments/probe_checkpoints/qwen3-vl-8b/mention_the \
